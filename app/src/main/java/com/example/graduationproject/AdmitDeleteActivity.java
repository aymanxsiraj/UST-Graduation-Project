...
public class AdmitDeleteActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admit_delete);
            ...
            user.delete()
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            DeleteAssociatedDataFromFirebaseRealtimeDatabaseDoctorsRoot(getIntent().getStringExtra("UID"));
                            DeleteAssociatedDataFromFirebaseRealtimeDatabaseStatusRoot(getIntent().getStringExtra("UID"));
    ...
    private void DeleteAssociatedDataFromFirebaseRealtimeDatabaseDoctorsRoot(String UID){
        DatabaseReference userRef = FirebaseDatabase.getInstance("https://graduation-project-6b165-default-rtdb.asia-southeast1.firebasedatabase.app/")
                .getReference("Users").child("Doctors").child(UID);
        userRef.removeValue()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
